...
<body>
    ...
    @if(isset($donation) && $donation->status === 'scheduled')
    <p>Your appointment is on {{ $donation->appointment_date }} at {{ $donation->appointment_time }}</p>

    <form method="POST" action="{{ route('donor.cancel-donation', $donation->id) }}">
        @csrf
...
@endif
        <div class="dashboard"> 
            ...
            <div class="blood-box"> 
                <h2>Request Accepts</h2> 
                <p class="heading">Total: {{ $stats['accepted'] ?? 0 }}</p>
            </div>
            <div class="blood-box"> 
                <h2>Request Rejects</h2> 
...
.then(response => {
    console.log("Authenticated Donor:", response.data);
    // You can now display donor info on your dashboard
})
.catch(error => {
